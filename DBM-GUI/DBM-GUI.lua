	...
	function DBM_GUI:UpdateModList()
		for k,addon in ipairs(DBM.AddOns) do
			if not Categories[addon.category] then
				...
				if tonumber(WowBuild) >= 13206 then--Choose default expanded catagory based on build. expands cata in cata and wotlk if not cata. Use build number instead of toc so 4.0.1 is treated as wrath too.

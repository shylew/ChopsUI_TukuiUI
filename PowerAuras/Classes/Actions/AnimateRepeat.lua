...
--- Animate action, runs a looped animation upon a display when certain
--  criteria are met.
local AnimateRepeat = PowerAuras:RegisterActionClass("AnimateRepeat", {
	Parameters = {
		[1] = 1,
	},
});
...
--                animations on.
function AnimateRepeat:New(id, parameters, display, channel)
	-- Generate the activator.
	return function(seqID, oldSeqID, animation)
		if(seqID and seqID ~= oldSeqID) then
			display:QueueAnimation(animation, true, channel);
		elseif(not seqID and oldSeqID) then
			display:QueueAnimation(nil, true, channel);
		end
	end;
end
...
function AnimateRepeat:CreateSequenceEditor(frame, ...)
	-- Construct widgets.
...
function AnimateRepeat:Upgrade(version, params)

...
local Timer = PowerAuras:RegisterServiceImplementation("UnitAura", "Timer", {
...
function Timer:New(parameters)
	...
	return function()
		...
		if(not result) then
			...
			local _, _, _, _, _, dur, exp = UnitAura(matchUnit, index, filter);
			dur, exp = (dur or min), (exp or min);
			return (exp - dur), (exp > min and exp or max);
		end
	end
end

...
local Text = PowerAuras:RegisterServiceImplementation(
	"TriggerData", "Text", {
...
function Text:New(parameters)
	...
	return function(subs)
		...
		if(type(store) == "table" and type(store.Text) == "table") then
			for k, v in pairs(store.Text) do
				subs[k] = v;
			end
		end
		return subs;
...
function Text:CanCreateFromTrigger(id, index)
...
function Text:CreateFromTrigger(tp, out, id, index)
...
function Text:GetActionDependencies(params)

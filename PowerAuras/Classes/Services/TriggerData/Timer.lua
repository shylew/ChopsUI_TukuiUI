...
local Timer = PowerAuras:RegisterServiceImplementation(
	"TriggerData", "Timer", {
...
function Timer:New(parameters)
	...
	return function()
		...
		if(type(store) == "table") then
			return (store.TimerStart or min), (store.TimerEnd or max);
		else
			return min, max;
		end
	end
...
function Timer:CanCreateFromTrigger(id, index)
...
function Timer:CreateFromTrigger(tp, out, id, index)

...
local PT3, oldminor = LibStub:NewLibrary("LibPeriodicTable-3.1", tonumber(("$Revision: 386 $"):match("(%d+)")) + 90000)
if not PT3 then
	return
end
...
---------------------------------------------
local getItemID, makeNonPresentMultiSet, shredCache, setiter, multisetiter, fixSetName
...
function fixSetName(set)
	-- strip comma from set names since it's used as special char in multisets
	return (set:gsub(",", ""))
end
